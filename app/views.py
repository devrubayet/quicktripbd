...
from .models import AirlinesList, Our_Services_Slider
...
def index(request):
    ...
    ourservices_slider = Our_Services_Slider.objects.filter(is_active=True).order_by('-created_at')
    context={
        'info':info,
        'images':images,
        'visa_list':visa_list,
        ...
        'feedbacks':feedbacks,
        'ourservices_slider':ourservices_slider
...
def package_details(request, slug):
    ...
    package_list = PackageList.objects.all()
    package_details = get_object_or_404(PackageList, slug=slug)
    bank_details = BankDetails.objects.all()
    visa_list = VisaList.objects.all()
    images = Slider.objects.filter(is_active=True).order_by('-created_at')
